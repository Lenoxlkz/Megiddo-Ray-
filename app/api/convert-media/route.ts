...
import { extractInstagramMedia, extractTikTokMedia, extractTwitterMedia, extractYouTubeMedia, extractFacebookMedia, extractThreadsMedia } from '@/lib/mediaExtractor';
import fs from 'fs';
...
async function handleMediaConversion(params: {
  ...
  try {
    ...
    if (type === 'mp3' || type === 'audio') {
      ...
      } else if (mediaStreamUrl.includes('threads.com') || mediaStreamUrl.includes('threads.net')) {
        const thData = await extractThreadsMedia(mediaStreamUrl);
        if (thData.videoUrl) {
          mediaStreamUrl = thData.videoUrl;
        } else {
          return NextResponse.json({
            error: 'No se detectó pista de audio o flujo de video en esta publicación de Threads para exportar como MP3.'
          }, { status: 400 });
        }
      }
