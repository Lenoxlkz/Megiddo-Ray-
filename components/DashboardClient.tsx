...
export default function DashboardClient() {
      ...
      if (sharedUrl || sharedText || sharedTitle) {
        ...
        setTimeout(() => {
          if (extracted.isValid && extracted.normalizedUrl) {
            ...
            showToast('Enlace recibido desde el menú Compartir');
          } else {
            setShowNewModal(true);
            addAlert({
              type: 'url_invalid',
            ...
            showToast('Contenido recibido. Verifica el enlace.');
          }
        }, 0);
      }
    } catch (err) {
  ...
  }, [cleanInputUrl, analyzeUrlInput, addAlert, showToast]);
  ...
  const addTracker = async (categoryOverride?: SearchCategory) => {
    ...
    if (detectionResult?.needsConfirmation && !userConfirmedCategory && !categoryOverride) {
      showToast(language === 'es' ? 'Por favor confirma la categoría antes de iniciar' : 'Please confirm category before proceeding');
      return;
    }
    ...
    setDetectionResult(null);
    setUserConfirmedCategory(false);
    showToast(`${chosenCategory === 'video' ? 'Video' : chosenCategory === 'image' ? 'Imagen' : 'Manga'} añadido con éxito`);
                          ...
                          <div className="min-w-0">
                            <div className="flex items-center gap-1.5 flex-wrap">
                              ...
                              <span className="px-2 py-0.5 rounded-full text-[10px] font-mono font-bold bg-emerald-400/20 text-emerald-300 border border-emerald-400/40 uppercase">
                                {detectionResult.category === 'manga' ? 'Manga / Cómic' : detectionResult.category === 'video' ? 'Video' : 'Imagen'}
                              </span>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <div className="font-bold text-amber-200 text-xs sm:text-sm">
                              {language === 'es' 
                                ...
                                : `Detected: ${detectionResult.platformName} • Suggested: ${detectionResult.suggestedCategory.toUpperCase()}`}
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-1.5">
                            <button
                              ...
                              onClick={() => {
                                ...
                                setUserConfirmedCategory(true);
                                showToast(language === 'es' ? 'Categoría confirmada: Vídeo' : 'Category confirmed: Video');
                              }}
                              className={cn(
                            ...
                            <button
                              ...
                              onClick={() => {
                                ...
                                setUserConfirmedCategory(true);
                                showToast(language === 'es' ? 'Categoría confirmada: Imagen' : 'Category confirmed: Image');
                              }}
                              className={cn(
                                "px-2.5 py-1 rounded-lg text-xs font-bold transition-all cursor-pointer flex items-center gap-1 border",
                            ...
                            <button
                              ...
                              onClick={() => {
                                ...
                                setUserConfirmedCategory(true);
                                showToast(language === 'es' ? 'Categoría confirmada: Manga' : 'Category confirmed: Manga');
                              }}
                              className={cn(
                                "px-2.5 py-1 rounded-lg text-xs font-bold transition-all cursor-pointer flex items-center gap-1 border",
                        ...
                        <div className="flex items-center gap-2 pt-1">
                          ...
                          <button
                            ...
                            onClick={() => {
                              setNewCategory('image');
                              setUserConfirmedCategory(true);
                              showToast('Modo genérico: Imagen');
                            }}
                            className={cn(
                              "flex-1 py-1.5 px-2 rounded-xl text-xs font-semibold border flex items-center justify-center gap-1.5 transition-all cursor-pointer",
                              newCategory === 'image' && userConfirmedCategory
